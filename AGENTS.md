...
- **Consent**: headers come from `SimulaPrivacy` at request time; PII is re-read at flush, never cached — GAID from the live consent snapshot (gated by COPPA / ad-id opt-in), PPID from the live identity provider (not consent-gated in the pipeline today; `SimulaPrivacyConfig.allowsPrimaryUserID` is defined but unwired — see `docs/SDK_IMPROVEMENT_PLAN_V2.md`).
