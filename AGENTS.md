...
- New console logging (`Log.*`) — operational signals go to telemetry (`Telemetry.recordOperation`/`recordError`); the dev-mode Logcat mirror is the only sanctioned console output
...
- **Deferred startup**: `SimulaAds.initialize` keeps the calling thread (usually main, from `Application.onCreate`) free of disk I/O — `SimulaPrivacy.attach`, `Telemetry.initialize` (SQLite), the initial GAID refresh, the `AdBeaconManager.init` build, the recovery/beacon drains, `WebViewPool.prewarm` (skipped when `ActivityManager.isLowRamDevice` — see `shouldSkipStartupPrewarm`; normal pooling after an ad request is never gated), and the session warm-up run in the `SimulaScope` startup block in that order; `SimulaCrashGuard.install` runs as an **ungated follow-up** after telemetry install (its replay records into the telemetry manager, so it must follow it) — it hops to the main thread and the ad-request gate must never wait on main-thread health. `SimulaSessionStore.startupGate` holds any host load fired immediately after `initialize` until consent + telemetry + the beacon queue are in place (the queue build is inside the gate because a released load can fire billing beacons immediately, and `enqueue` is a no-op until the engine exists). Keep heavy one-time work there, not inline in `initialize`.
- **Uninitialized SDK**: return null / render blank / record one telemetry note (never console-log). `require()` only at documented init boundaries.
...
- **Consent**: headers come from `SimulaPrivacy` at request time; PII is re-read at flush, never cached — GAID from the live consent snapshot (gated by COPPA / ad-id opt-in), PPID from the live identity provider (not consent-gated in the pipeline today; `SimulaPrivacyConfig.allowsPrimaryUserID` is defined but unwired — see `docs/SDK_IMPROVEMENT_PLAN.md`).
