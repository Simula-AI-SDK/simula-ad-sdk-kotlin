...
The SDK collects handled-error and performance telemetry by default. Set `telemetryEnabled = false`
on `SimulaProvider` or `SimulaAds.initialize` to disable the pipeline entirely. Configuration is
first-registration-wins: the first entry point to run (imperative or provider) fixes this value for
the process, and changing it at runtime has no effect.
